...
#proof[
  ...
  (Предположим $alpha_i = 0 -> x_i = alpha_i e_1 = 0_V -> x_1, x_2, ..., x_i, ..., x_k ->$ линейно завис. система (т.к. содержит $0_V$) $->$ противоречие с тем, что система линейно независима $-> alpha_i != 0$).
  ...
  $ 1 / alpha_1 x_1 = 1 / alpha_2 x_2 => overbracket(1 / alpha_1, !=0) x_1 + overbracket((- 1 / alpha_2), !=0) x_2 = 0_V <-> x_1 text(" и ") x_2 text(" линейно завис.") $
  (сверху над левой частью подписано: нетривиальн.)
  
  $ -> x_1, x_2, ..., x_k text(" --- линейно завис. (т.к. содержит ЛЗ подсистему)") -> $
  получили противоречие $-> k >= 2$ --- неверно $-> k=1$.

  2) Пусть $n >= 2$. \
  Индукт. предположение. Если $k'$ линейно независимых векторов линейно выражаются через векторы $e_1, e_2, ..., e_(n-1)$, то $k' <= n-1$.
  ...
  Покажем, что $k <= n$. \
  Если $alpha_(1n) = alpha_(2n) = ... = alpha_(k n) = 0$, то линейно независ. векторы $x_1, x_2, ..., x_k$ линейно выражаются через векторы $e_1, e_2, ..., e_(n-1)$ и по индукт. предположению $k <= n-1 -> k <= n$.
  ...
  Векторы $x_1 - alpha_(1 n) / alpha_(k n) x_k, quad x_2 - alpha_(2 n) / alpha_(k n) x_k, quad ..., quad x_(k-1) - alpha_((k-1) n) / alpha_(k n) x_k$ \
  (линейно независ. $k-1$ векторов) \
  линейно выражаются через векторы $e_1, e_2, ..., e_(n-1)$. \
  Остается показать, что они линейно независимы. \
  По индукт. предполож. $k-1 <= n-1 -> k <= n ->$ доказали бы теорему.
