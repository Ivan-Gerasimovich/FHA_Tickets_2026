...
#definition[
  ...
  *Скалярным произведением* векторов в линейном пространстве $V$ наз-ся отображение:
  ...
  Свойства $2^circle$ и $3^circle$ --- _свойство линейности по 1-му аргументу._

== Примеры:

*1)* Обычное скалярное произведение в $V_2$ или $V_3$ удовлетворяет $1^circle - 4^circle$ (стандартное скалярное произведение).

*2)* $RR^n$, $"   "$$forall x in RR^n quad forall y in RR^n quad (x, y) = x_1 y_1 + x_2 y_2 + ... + x_n y_n = sum_(i=1)^n x_i y_i = x^T y$. \
// TODO: Проверить выполнимость аксиом $1^circle - 4^circle$ и добавить.

*3)* Рассмотрим $A in RR^(n times n)$. Определим для $forall x in RR^n quad forall y in RR^n$:
...
#property(title: "Утверждение")[
  Формула $"(*)"$ определяет скалярное произведение на $RR^n <=> $ матрица $A$ симметричная и положительно определённая.
]
\

Рассматривается пространство $RR^n$

$(x, y) = x^T dot A dot y quad "- скалярное произведение" <=> \ <=> "A - cимметрическая" (A = A^T) "и положительно определенная"  [forall x in RR^n \\ {0} quad x^T A x > 0]. $
